...
public class MySqlSupporter extends DbmsSupporter {
	
	private static final Logger log = LoggerFactory.getLogger(MySqlSupporter.class);
	
	public MySqlSupporter() {
		super("mysql");
	}
	...
	public boolean match(Connection conn) throws SQLException {
		...
		if (drivername.toUpperCase().indexOf("MYSQL") >= 0) {
			if (log.isDebugEnabled()) {
				log.debug("MySQL JDBC Driver : "+drivername+" v"+dbmeta.getDriverMajorVersion()+"."+dbmeta.getDriverMinorVersion());
				log.debug("MySQL Server Product : "+dbmeta.getDatabaseProductName()+" "+dbmeta.getDatabaseProductVersion());
				log.debug("MySQL Connection URL : "+dbmeta.getURL()+" ("+dbmeta.getUserName()+")");
