...
public class PostgresSupporter extends DbmsSupporter {
	
	private static final Logger log = LoggerFactory.getLogger(PostgresSupporter.class);
	...
	public boolean match(Connection conn) throws SQLException {
		...
		if (drivername.toUpperCase().indexOf("POSTGRESQL") >= 0) {
			if (log.isDebugEnabled()) {
				log.debug("PostgreSQL JDBC Driver : "+drivername+" v"+dbmeta.getDriverMajorVersion()+"."+dbmeta.getDriverMinorVersion());
				log.debug("PostgreSQL Server Product : "+dbmeta.getDatabaseProductName()+" "+dbmeta.getDatabaseProductVersion());
				log.debug("PostgreSQL Connection URL : "+dbmeta.getURL()+" ("+dbmeta.getUserName()+")");
